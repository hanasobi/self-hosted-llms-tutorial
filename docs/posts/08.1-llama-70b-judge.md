...
**Wichtig:** Dieser Post fokussiert sich auf **Inference Performance** (LLM-as-Judge Evaluations). Training kommt eventuell in einem Follow-up.
...
**Wichtig:** `num_gpu 81` lädt alle 81 Transformer Layers auf Metal (Apple GPU). Bei initialen Tests hatte wir aus Vershen nur 34 Layers auf GPU geladen - explizites Setzen auf 81 brachte 140% Performance-Gewinn (siehe Performance-Section). Der Default von -1 (auto) hat in unseren Tests, genauso funktioniert.
...
**Wichtige Einschränkung:** Dieser Post fokussiert sich auf **Inference Performance**. Die überraschend gute Performance wirft aber die Frage auf: **Wie sieht's mit Training aus?** Könnte LoRA Fine-Tuning aus Post 5 auch lokal auf Mac Studio laufen? Das schauen wir uns eventuell in einem Follow-up an.
